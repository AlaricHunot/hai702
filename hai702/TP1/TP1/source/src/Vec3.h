...
public:
    ...
    static float dot(Vec3 const &a, Vec3 const &b) {
        ...
        float res=a[0]*b[0]+a[1]*b[1]+a[2]*b[2]; //Faire le calcul ici et assigner le resultat à res
        return res;
    }
    ...
    static Vec3 cross(Vec3 const &a, Vec3 const &b) {
        ...
        res[2]=a[0]*b[1]-a[1]*b[0];
         //Faire le calcul ici et assigner le resultat à res
        return res;
    }
...
public:
    ...
    Vec3 operator*(const Vec3 &p) {
        //TODO: Fonction à completer
        //Pour acceder a un element de la matrice (*this)(i,j) et du point p[i]
        Vec3 res = p; //Faire le calcul ici et assigner le resultat à res
        for(int i=0;i<3;i++){
            res[i]=(*this)(i,0)+p[0]+(*this)(i,1)+p[1]+(*this)(i,2)+p[2];
        }
        return res;
    }
    ...
    Mat3 operator*(const Mat3 &m2) { // calcul du produit matriciel m1.m2
        ...
        Mat3 res; //Faire le calcul ici et assigner le resultat à res
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                res(i,j)=(*this)(i,0)*m2(0,j)+(*this)(i,1)*m2(j,1)+(*this)(i,2)*m2(2,j);
            }
        }
        return res;
    }
